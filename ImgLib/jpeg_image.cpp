...
bool SaveJPEG(const Path& file, const Image& image) {
    ...
    */
    ...
    const img_lib::Color* line =   image.GetLine(cinfo.next_scanline);
    for (int x = 0; x < cinfo.image_width; ++x) {
        image_buffer[x * 3 + 0] = static_cast<JSAMPLE>(line[x].r);
        image_buffer[x * 3 + 1] = static_cast<JSAMPLE>(line[x].g);
        image_buffer[x * 3 + 2] = static_cast<JSAMPLE>(line[x].b);
    }

    JSAMPLE* jsp = image_buffer.data();
    row_pointer[0] = jsp;
